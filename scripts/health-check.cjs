...
async function checkDatabase() {
    ...
    try {
        ...
        const requiredTables = [
            'users', 'farms', 'lots', 'pest_monitoring', 'inputs',
            'inventory', 'agricultural_tasks', 'input_usage',
            'expenses', 'harvests', 'processing', 'microlots',
            'quality_controls', 'traceability_events',
            'certification_records', 'generated_documents'
        ];
        ...
        const [fincaCount] = await connection.execute('SELECT COUNT(*) as count FROM farms');
